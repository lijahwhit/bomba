...
class GameState:
	...
	def __init__(self, is_over:bool, tick_number:int, size:Point, 
				...
				treasure:List[Point],
				bombs:List[Point],
				blocks:List[Tuple[EntrtyTags, Point]],
				players:List[Tuple[ID, Point]],
				):
	...
	def indestructible_blocks(self) -> List[Point]:
		return [pos for tag, pos in self._blocks if tag == EntrtyTags.IndestructibleBlock.value]

	@property
	def soft_blocks(self) -> List[Point]:
		return [pos for tag, pos in self._blocks if tag == EntrtyTags.SoftBlock.value]
	
	@property
	def ore_blocks(self) -> List[Point]:
		return [pos for tag, pos in self._blocks if tag == EntrtyTags.OreBlock.value]
	...
	def entity_at(self, location:Point) -> EntrtyTags:
	...
	def opponents(self, excluding_player_pid:ID=None):
...
class PlayerState:
	def __init__(self, id:ID, ammo:int, hp:int, location:Point, reward:int, power:int):
		self.id = id
		self.ammo = ammo
		self.hp = hp
